<?php
// =================================================================
// LOGIKA UNTUK HALAMAN RIWAYAT GAJI
// =================================================================
    ...
    try {
        switch ($action) {
            case 'edit_gaji_riwayat':
                $sql = "UPDATE riwayat_gaji SET tanggal = ?, berat_barang_kg = ?, tarif_per_kg = ?, total_gaji = ?, keterangan = ? WHERE id_riwayat = ?";
...
<main class="flex-1 bg-gray-100">
            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php foreach ($riwayat_list as $item): ?>
                        <tr class="border-b">
                            <td class="p-2"><?php echo htmlspecialchars(date('d M Y', strtotime($item['tanggal']))); ?></td>
                            <td class="p-2"><?php echo htmlspecialchars($item['berat_barang_kg']); ?> Kg</td>
                            <td class="p-2">Rp. <?php echo number_format($item['total_gaji'], 0, ',', '.'); ?></td>
                            <td class="p-2"><?php echo htmlspecialchars($item['keterangan']); ?></td>
                            <td class="p-2 space-x-1">
                                <button class="btnEditGaji bg-blue-500 text-white text-xs px-2 py-1 rounded" data-id-riwayat="<?php echo $item['id_riwayat']; ?>" data-tanggal="<?php echo $item['tanggal']; ?>" data-berat="<?php echo $item['berat_barang_kg']; ?>" data-keterangan="<?php echo $item['keterangan']; ?>">Edit</button>
                                <button class="btnHapusGaji bg-red-500 text-white text-xs px-2 py-1 rounded" data-id-riwayat="<?php echo $item['id_riwayat']; ?>">Hapus</button>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                <?php endif; ?>
    ...
    <div id="modalEditGaji" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 flex">
        <form action="Index.php?page=riwayat_gaji&id_pekerja=<?php echo $id_pekerja; ?>" method="POST" class="bg-white p-6 shadow-md rounded w-80 relative">
            ...
            <input type="hidden" name="action" value="edit_gaji_riwayat"><input type="hidden" name="id_riwayat_edit" id="id_riwayat_edit"><input type="hidden" name="tarif_per_kg" id="edit_tarif_per_kg" value="2500"><input type="hidden" name="total_gaji" id="edit_total_gaji_hidden">
            <div class="mb-4"><label for="edit_tanggal" class="block text-sm font-medium text-gray-700 mb-1">Tanggal</label><input type="date" name="tanggal" id="edit_tanggal" class="w-full px-3 py-2 border border-gray-300 rounded" required /></div>
            <div class="mb-4"><label for="edit_berat_barang_kg" class="block text-sm font-medium text-gray-700 mb-1">Berat (Kg)</label><input type="number" step="0.1" name="berat_barang_kg" id="edit_berat_barang_kg" class="w-full px-3 py-2 border border-gray-300 rounded" required /></div>
            <div class="mb-4 text-sm">Total Gaji: <span class="font-mono font-bold" id="edit_total_gaji_display"></span></div>
            <div class="mb-6"><label for="edit_keterangan" class="block text-sm font-medium text-gray-700 mb-1">Keterangan</label><select name="keterangan" id="edit_keterangan" class="w-full px-3 py-2 border border-gray-300 rounded">
    ...
    <div id="modalHapusGaji" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 flex">
        <div class="w-[320px] border p-6 bg-white rounded-md relative">
            ...
            <form action="Index.php?page=riwayat_gaji&id_pekerja=<?php echo $id_pekerja; ?>" method="POST" class="flex justify-end space-x-3">
                <input type="hidden" name="action" value="hapus_gaji_riwayat"><input type="hidden" name="id_riwayat_hapus" id="id_riwayat_hapus">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        document.querySelectorAll('.btnEditGaji').forEach(button => {
            button.addEventListener('click', e => {
                ...
                modalEditGaji.querySelector('#id_riwayat_edit').value = data.idRiwayat;
                modalEditGaji.querySelector('#edit_tanggal').value = data.tanggal;
                modalEditGaji.querySelector('#edit_berat_barang_kg').value = data.berat;
                modalEditGaji.querySelector('#edit_keterangan').value = data.keterangan;
                modalEditGaji.querySelector('#edit_berat_barang_kg').dispatchEvent(new Event('input'));
        ...
        document.querySelectorAll('.btnHapusGaji').forEach(button => {
            button.addEventListener('click', e => {
                const data = e.target.closest('button').dataset;
                modalHapusGaji.querySelector('#id_riwayat_hapus').value = data.idRiwayat;
